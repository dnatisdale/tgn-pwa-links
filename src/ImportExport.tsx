...
function parseJson(text: string): RowIn[] {
  try {
    ...
    if (obj && Array.isArray(obj.items)) return obj.items as RowIn[];
    return [];
  } catch {
    return [];
  }
...
export default function ImportExport({ lang }: { lang: Lang }) {
  ...
  const validItems = useMemo(() => preview.filter(p => p.valid && p.urlHttps), [preview]);
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const pv: PreviewItem[] = rows.map((r, idx) => {
  ...
  async function doImport() {
    setImportMsg("");
    if (!auth.currentUser) { setImportMsg("Not signed in."); return; }
    const uid = auth.currentUser.uid;
    const col = collection(db, "users", uid, "links");

    const items = validItems;
    if (items.length === 0) { setImportMsg("Nothing valid to import."); return; }
      ...
      for (let i = 0; i < items.length; i += CHUNK) {
        ...
        for (const it of chunk) {
          ...
          batch.set(ref, {
            ...
            importBatchId: Date.now(),            // basic tag you can use to undo
          });
        }
        await batch.commit();
      }
      setImportMsg(`Imported ${items.length} item(s).`);
      // Clear preview if you want:
      // setPreview([]);
    } catch (e: any) {
  ...
  // Export helpers (kept simple)
  function exportCSV() {
    const rows = preview.length ? preview : [];
    const head = ["name", "language", "url"];
    const body = rows.map(r => [r.name, r.language, r.urlRaw || r.urlHttps || ""].map(v => `"${(v||"").replace(/"/g,'""')}"`).join(","));
      ...
      {!!preview.length && (
        <div className="mt-4">
          <div className="text-sm mb-2">
            ...
            <span style={{ color: "#b91c1c" }}>{preview.length - validItems.length} invalid</span>
          </div>
